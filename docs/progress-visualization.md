...
The visible division window is derived from the player's own seasons with one neighboring division of context on either side. The time axis is likewise derived from the player's season and checkpoint dates with modest padding, rather than showing the entire national division scale or an unrelated calendar range.
